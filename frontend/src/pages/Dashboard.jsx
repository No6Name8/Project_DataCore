  ...
  return (
    <div className="glass-card rounded-xl p-5 space-y-3 animate-pulse">
      <div className="flex items-center justify-between">
        ...
        <div className="w-14 h-5 rounded-full bg-surface-border" />
      </div>
      <div>
        <div className="h-8 bg-surface-border rounded w-2/3 mb-1.5" />
        ...
        <table className="w-full text-xs">
          ...
          <tbody className="divide-y divide-surface-border">
            {transactions.slice(0, 8).map((tx, i) => (
              <tr key={i} className="hover:bg-surface-hover" style={{ transition: "background-color 120ms var(--ease-out)" }}>
    ...
    <div className="glass-card rounded-xl p-5 space-y-4">
      ...
      <div className="h-3 rounded-full overflow-hidden flex">
        {bars.map((b, i) => (
          <div key={i}
            className={b.color}
            style={{ width: `${b.value * 100}%`, transition: "width 600ms var(--ease-out)" }} />
        ))}
      </div>
...
export default function Dashboard() {
  ...
  const selectedBizObj = BUSINESSES.find(b => b.id === selectedBiz);
  ...
  return (
    <div className="space-y-5">
      ...
      {/* Business selector — sliding layoutId indicator */}
      <div className="flex gap-2 overflow-x-auto pb-1">
        {BUSINESSES.map(biz => (
          <button
            key={biz.id}
            onClick={() => handleBizChange(biz.id)}
            className={`relative shrink-0 px-4 py-2 rounded-lg text-sm
      ...
      {/* Business name + archetype header */}
      <div className="flex items-center gap-3 min-h-[2.5rem]">
        <div>
          <h2 className="text-base font-semibold text-white">
            {isRTL ? selectedBizObj?.nameAr : selectedBizObj?.nameEn}
          </h2>
          {dscr?.archetype_description && dscr.archetype_description !== "unknown" && (
            <p className="text-xs text-brand-gold mt-0.5">{dscr.archetype_description}</p>
          )}
        </div>
      </div>
          ...
          <>
            <motion.div custom={0} variants={cardVariants} initial="hidden" animate="visible">
              <KPICard
                ...
                value={summary
                  ? <>
                      <span className="text-xs text-gray-400 me-0.5">SAR</span>
                      <AnimatedNumber value={summary.total_revenue_sar} />
                    </>
                  : "—"}
                sub={t.period}
